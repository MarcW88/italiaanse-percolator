# Sources and attribution

The skills in this repository are site-specific adaptations inspired by public agent-skill projects.

## RampStack Co. — claude-skills

Source: https://github.com/rampstackco/claude-skills

Concepts adapted from skills including:
- seo-content-audit
- content-refresh-system
- evidence-based-reviews
- seo-onpage
- editorial-qa

License: MIT License, Copyright (c) 2026 RampStack Co.

## Conor Bronsdon — avoid-ai-writing

Source: https://github.com/conorbronsdon/avoid-ai-writing

The natural-writing skill is an original Dutch/site-specific adaptation informed by the pattern-audit approach of avoid-ai-writing.

License: MIT License, Copyright (c) 2026 Conor Bronsdon.

## jwynia — agent-skills fact-check

Source: https://github.com/jwynia/agent-skills

The fact-check skill is an adapted, simplified site-specific version of the separate-pass verification methodology. The source SKILL.md declares license: MIT.

## Adaptation

The files under `.agents/skills/` are not verbatim copies. They have been rewritten and narrowed specifically for italiaanse-percolator.nl, Dutch-language affiliate content, mokapots/percolators, product comparisons and SEO recovery.


## Additional external skills

- dageno-agents/internal-linking-audit — internal-linking-audit/SKILL.md
- rampstackco/claude-skills — seo-technical/SKILL.md
- AsyrafHussin/agent-skills — seo-best-practices/SKILL.md
- seranking/seo-skills — seo-page/SKILL.md and seo-drift/SKILL.md

These are stored as upstream skill files under .agents/skills/. Their original license/frontmatter is preserved in the copied files.
